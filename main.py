...
def main(page: ft.Page):
    page.title = "Tjatindi Michael Kazundire - SiteSpy Portfolio"
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("Tjatindi Michael Kazundire", size=34, weight=ft.FontWeight.W_900, color=TEXT),
            ft.Text("GitHub: IcyBeibi68023 | Email: michaelkazundirejax@gmail.com", color=MUTED, selectable=True),
            ft.Text("Security rules and Firebase access documentation.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/IcyBeibi68023", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Security rules and Firebase access documentation.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- firestore.rules', color=MUTED, size=13),
                    ft.Text('- storage.rules', color=MUTED, size=13),
                    ft.Text('- docs/Firebase_Setup_Guide.md', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Tjatindi Michael Kazundire\"\ngit config user.email \"michaelkazundirejax@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/tjatindi-mk-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Tjatindi Michael Kazundire SiteSpy portfolio\"\ngit push -u origin portfolio/tjatindi-mk-sitespy-portfolio", color=TEXT, selectable=True),
